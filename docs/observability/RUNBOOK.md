...
Traces (opcional): `OTEL_EXPORTER_OTLP_ENDPOINT` → collector OTLP/HTTP. Logs Pino incluem `requestId`, e `trace_id` / `span_id` quando há span ativo. Respostas de erro trazem `errorId` + `requestId` + `traceId`. Header `X-Request-Id` (entrada opcional / eco na resposta).

Graceful shutdown: `enableShutdownHooks()` no Nest + `sdk.shutdown()` OTEL em `SIGTERM`.

Labels de baixa cardinalidade apenas (`method`, `route` normalizada, `status_class`, `operation`, `result`, `code`, `breaker`, `event`, `resource`). Nunca UUID, documento, URL crua ou chave de cache.
...
| Erros 5xx | > 2% por 5m (`Http5xxRate`) | `sum(rate(http_requests_total{status_class="5xx"}[5m]))` + logs `requestId` / `errorId` / `trace_id` | Abrir log do `errorId` (ou `requestId`); corrigir causa; confirmar `/health/ready`. |
| Banco indisponível | `db_up==0` por 1m (`DbDown`) | `db_up == 0` (probe curto no scrape `/metrics` + `/health/ready`) | Verificar Postgres (`docker compose ps`), `DATABASE_URL`, rede/porta 5433. |
...
| Job PENDING falhando | item error rate > 0.05/s por 15m (`PendingRevalidateErrors`) | `sum(rate(pending_revalidate_items_total{result="error"}[15m]))` | Checar logs `{ job: pending_external_validation }`; BrasilAPI; `REVALIDATE_PENDING_*`; admin revalidate. |
